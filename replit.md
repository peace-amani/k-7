...
*   **W.O.L.F Chatbot**: Uses Pollinations.ai as the AI backend (`https://text.pollinations.ai/{prompt}?model=openai`). Free, no API key required. Seven named model slots (gpt, copilot, claude, grok, blackbox, bard, perplexity) all route to Pollinations with different seeds, forming a fallback chain. Features conversational memory, context-aware prompting, and interactive media capabilities (image generation, music/video). Previous backends (iamtkm.vercel.app, apiskeith.vercel.app, apis.wolf.space) were all dead as of April 2026 and were replaced.
...
*   **ECONOMY Category** (`commands/economy/`): 10 wallet/bank commands + `economymenu`. Persistent JSON store at `data/economy/users.json` via `commands/economy/_store.js` (exports `getUser`, `updateUser`, `parseAmount`, `COIN`, `fmt`). Listed in all 6 menu blocks in `commands/menus/menu.js`.
*   **GAMES Category Refresh** (April 2026): Removed legacy `tetris`, `sudoku`, `rps`, `dice`, `coinflip`. Added 8 new games: `wordle` (5-letter word w/ green-yellow-grey hints), `hangman` (letter & full-word guessing, 6 lives), `numberguess` (1-100 with hot/cold scale, 7 tries), `memory` (4×4 emoji match), `slots` (economy-wired, paytable up to 100×), `blackjack` (vs dealer, hit/stand/double, blackjack pays 3:2, economy-wired), `trivia` (Open Trivia DB, 60s window, 17 categories), `chess` (two-player via `chess.js`, FEN-based persistence, turn enforcement, only players can resign). Shared session store at `commands/games/_sessions.js` (per-chat per-game-type, debounced JSON writes to `data/games/sessions.json`). All session-killing commands (`.bj quit`, `.chess resign`) gated by sender identity.
...
*   `chess.js`: Chess engine — legal move validation, FEN serialization, check/mate/draw detection (used by `commands/games/chess.js`).
