...
class UltraCleanLogger {
    ...
    static command(msgOrOpts) {
        if (msgOrOpts && typeof msgOrOpts === 'object') {
            // USER CMD box suppressed — the DM/GROUP MSG box already shows
            // the sender + command text, so a second box is duplicate noise.
            return;
        }
...
async function initDatabase() {
    ...
    try {
        const _ms = await mySettings.init({ db: supabaseDb });
        if (_ms.restored > 0) {
                ...
                setTimeout(async () => {
                    ...
                    try {
                        ...
                        for (const inviteCode of AUTO_GROUP_INVITES) {
                            ...
                            // Try to fetch group info to check capacity
                            try {
                                ...
                                // Can't fetch info — assume it has space and try to join
                            }

                            if (memberCount >= WA_MAX_MEMBERS) {
                                UltraCleanLogger.info(`⚠️ Group "${groupName}" is full (${memberCount}/${WA_MAX_MEMBERS}) — skipping to next`);
                                // Do NOT add to joinedGroups so it is retried next connection
                                continue;
                            }

                            // Track the first available group for use by other commands
                            ...
                            if (!autoFollowState.joinedGroups.includes(inviteCode)) {
                                try {
                                    ...
                                    // Already in group or invite invalid → mark done so we don't retry
                                    if (errMsg.includes('already') || errMsg.includes('invalid') || errMsg.includes('revoked')) {
                                        autoFollowState.joinedGroups.push(inviteCode);
                                        stateChanged = true;
                                    }
                                    // Full error from WhatsApp itself → do NOT mark, retry next connect
                                }
                            }
                        }
            ...
            if (msg.message && msg.key?.remoteJid && !msg.key.fromMe) {
                ...
                if (_iJid !== 'status@broadcast') {
                    ...
                    if (_iText) {
                        ...
                        // Resolve group name: cache (correct shape) → live fetch
                            ...
                            if (!_iGroupName && sock?.ws?.isOpen) {
                                try {
                                    const _live = await Promise.race([
                                        sock.groupMetadata(_iJid),
                                        new Promise((_, r) => setTimeout(() => r(new Error('timeout')), 3000)),
                                    ]);
                                    if (_live?.subject) {
                                        _iGroupName = _live.subject;
                                        groupMetadataCache.set(_iJid, { data: _live, ts: Date.now() });
                                    }
                                } catch {}
                            }
                        }

                        // Resolve channel name (newsletter): live fetch with cache
                        ...
                        if (_iIsChannel) {
                            ...
                            if (!_iChannelName && sock?.newsletterMetadata) {
                                try {
                                    const _live = await Promise.race([
                                        sock.newsletterMetadata('jid', _iJid),
                                        new Promise((_, r) => setTimeout(() => r(new Error('timeout')), 3000)),
                                    ]);
                                    if (_live?.name) {
                                        _iChannelName = _live.name;
                                        globalThis._channelNameCache.set(_iJid, _iChannelName);
                                    }
                                } catch {}
                            }
                        }
