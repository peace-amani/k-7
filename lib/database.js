...
// Path where the last known bot_id is persisted across restarts.
// Stored in data/ (git-ignored) so settings load correctly before WhatsApp
// reconnects — otherwise the startup code can only read the 'default' rows.
...
try {
    if (fs.existsSync(LAST_BOT_ID_PATH)) {
        const saved = JSON.parse(fs.readFileSync(LAST_BOT_ID_PATH, 'utf8'));
...
class SqlJsAdapter {
    constructor(sqlJsDb, dbPath) {
        ...
        // Periodic flush every 3 seconds to avoid data loss on crashes
...
// Also persists the cleaned phone number to data/last_bot_id.json so the
// correct DB scope is available immediately on the next restart.
export function setConfigBotId(botId) {
    if (botId) {
        ...
        // Persist so the next restart can load settings before WhatsApp reconnects
        try {
            if (!fs.existsSync(DB_DIR)) fs.mkdirSync(DB_DIR, { recursive: true });
            fs.writeFileSync(LAST_BOT_ID_PATH, JSON.stringify({ botId: configBotId, savedAt: new Date().toISOString() }));
        } catch { /* non-fatal */ }
    }
}
...
// Prevents the .db-wal file from growing unboundedly on busy bots.
...
// Run VACUUM to reclaim disk space after bulk deletions.
// This rewrites the entire database file so it can take a few seconds.
// Run infrequently (once per 24 h) and only when the bot is not busy.
function runVacuum() {
    if (!isConnected || !db) return;
    try {
        db.exec('PRAGMA wal_checkpoint(TRUNCATE)');
