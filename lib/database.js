...
function restoreFromBackup() {
    ...
    try {
        ...
        restoreTx();
        if (restored > 0) console.log(`✅ SQLite: Restored ${restored} rows from backup`);
    } catch (err) {
...
export async function initTables() {
    try {
        ...
        for (const [name, sql] of Object.entries(TABLE_SCHEMAS)) {
            try { db.exec(sql); }
            catch (err) { console.error(`⚠️ SQLite: Error creating table ${name}:`, err.message); }
        }
        ...
        _backupTimer = setInterval(saveBackup, 5 * 60 * 1000);

        startPeriodicCleanup();

        console.log(`✅ SQLite: All tables ready — engine: better-sqlite3 (native WAL)`);
        return true;
    } catch (err) {
...
export async function get(table, key, keyColumn = 'key') {
    try {
        ...
        const row = db.prepare(`SELECT * FROM ${t} WHERE ${col} = ? LIMIT 1`).get(key);
...
export async function remove(table, key, keyColumn = 'key') {
    try {
        ...
        const t = validateTable(table);
        const col = validateColumn(keyColumn);
        db.prepare(`DELETE FROM ${t} WHERE ${col} = ?`).run(key);
        if (CRITICAL_TABLES.includes(t)) markBackupDirty();
        return true;
    } catch { return false; }
}
...
export async function removeWhere(table, filters = {}) {
    try {
        ...
        const conditions = keys.map(k => `${validateColumn(k)} = ?`);
        db.prepare(`DELETE FROM ${t} WHERE ${conditions.join(' AND ')}`).run(...Object.values(filters));
        if (CRITICAL_TABLES.includes(t)) markBackupDirty();
        return true;
    } catch { return false; }
}
...
export async function cleanOlderThan(table, timestampColumn, maxAgeMs) {
    try {
        ...
        const result = db.prepare(`DELETE FROM ${t} WHERE ${col} < ? AND bot_id = ?`).run(cutoff, botId);
...
export async function migrateJSONToConfig(filePath, configKey) {
    ...
    try {
        const botId = getConfigBotId();
        const existing = db.prepare('SELECT 1 FROM bot_configs WHERE key = ? AND bot_id = ? LIMIT 1').get(configKey, botId);
...
export async function clearAllAntideleteData() {
    ...
    try {
        ...
        try {
            const r2 = db.prepare('DELETE FROM antidelete_statuses WHERE bot_id = ?').run(botId);
...
const MAX_ANTIDELETE_AGE_MS = 12 * 60 * 60 * 1000;
const MAX_ANTIDELETE_ROWS = 300;
const VACUUM_INTERVAL = 6 * 60 * 60 * 1000;
...
export async function runPeriodicCleanup() {
    ...
    try {
        ...
        const total = msgCleaned + statusCleaned + rowLimitCleaned + mediaCleaned;
        if (total > 0) {
            saveBackup();
            console.log(`🧹 DB Cleanup: removed ${total} old records (msgs: ${msgCleaned}, statuses: ${statusCleaned}, rowLimit: ${rowLimitCleaned}, media: ${mediaCleaned})`);
        }
    } catch (err) {
...
export function startPeriodicCleanup() {
    ...
    setTimeout(runVacuum, 5 * 60 * 1000);
}
