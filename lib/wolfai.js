...
function quickIntentMatch(text) {
  ...
  // Menu
  if (/\b(show|open|get|display|see|view|give\s+me|bring\s+up|pull\s+up|can\s+i\s+see)\b.{0,25}\b(menu|commands?|command\s*list)\b/i.test(lower)
    || /\b(menu|commands?)\b.{0,10}(please|pls)?$/i.test(lower)
    || /^(show\s+)?(the\s+)?menu$/.test(lower)
    || /\bwhat\s+(commands?|can\s+you\s+do)\b/i.test(lower)) {
    return { command: 'menu', args: [] };
  }

  // Alive / status
  if (/\b(are\s+you\s+)?(alive|there|online|working|active|running|awake)\b/i.test(lower)
    || /\b(you\s+)?(still\s+)?(alive|there|up|working)\b/i.test(lower)) {
    return { command: 'alive', args: [] };
  }

  // Ping / speed
  if (/^ping$/.test(lower) || /\b(check\s+(the\s+)?ping|test\s+(the\s+)?speed|ping\s+test|latency)\b/i.test(lower)) {
    return { command: 'ping', args: [] };
  }

  // Uptime
  if (/\b(uptime|up\s*time|how\s+long\s+(have\s+you|has\s+(the\s+)?bot)\s+(been\s+)?(running|online|up)|runtime)\b/i.test(lower)
    || /^(uptime|up|runtime)$/.test(lower)) {
    return { command: 'uptime', args: [] };
  }

  // Prefix
  if (/\b(what\s+is\s+(the\s+)?(prefix|bot\s+prefix)|prefix\s*info|current\s+prefix|show\s+(the\s+)?prefix)\b/i.test(lower)
    || /^prefix(info)?$/.test(lower)) {
    return { command: 'prefixinfo', args: [] };
  }

  // Owner
  if (/^(show\s+)?(the\s+)?owner$/.test(lower) || /\bwho('s|\s+is)\s+(the\s+)?owner\b/i.test(lower)) {
    return { command: 'owner', args: [] };
  }

  // Restart
  if (/\b(restart|reboot)\b/i.test(lower)) {
    return { command: 'restart', args: [] };
  }

  // Mode switching
  const modeMap = {
    public:  /\b(public\s*mode|go\s*public|mode\s*(to\s*)?public|(switch|change|set)\s*(the\s*)?bot\s*mode\s*(to\s*)?public|everyone\s*mode)\b/i,
  ...
  const toggleable = 'antilink|antibug|antispam|antibadword|antileave|antiimage|antivideo|antisticker|antiaudio|antibot|antidelete|antiviewonce|antigrouplink|anticall|autotyping|autoread|autoviewstatus|autorecording|welcome|autoreact';
  const togRe = new RegExp(`(?:(?:turn|switch|set)\\s+)?(on|off|enable|disable)\\s+(?:the\\s+)?(${toggleable})|(?:the\\s+)?(${toggleable})\\s+(?:turn\\s+)?(on|off|enable|disable)`, 'i');
  const togM = lower.match(togRe);
  if (togM) {
    const cmd = (togM[2] || togM[3] || '').toLowerCase();
    const stateRaw = (togM[1] || togM[4] || '').toLowerCase();
    if (cmd && stateRaw) {
      return { command: cmd, args: [(stateRaw === 'on' || stateRaw === 'enable') ? 'on' : 'off'] };
    }
  }
