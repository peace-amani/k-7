...
function quickIntentMatch(text) {
  ...
  // Menu — only fire for explicit menu requests, not conversational questions
  if (/^(show\s+)?(me\s+)?(the\s+)?menu(\s+please|\s+pls)?$/.test(lower)
    || /^(open|pull\s+up|bring\s+up)\s+(the\s+)?menu$/.test(lower)
    || /^(show|display|get)\s+(the\s+)?(bot\s+)?menu$/.test(lower)
