...
function detectChatControlIntent(text, currentChatId) {
  ...
  // "I'll be speaking/talking/chatting to/with [this guy] 254713046497, do not interrupt"
  // Covers verbs: speak, talk, chat, message, text — and prepositions to/with
  // Also handles filler words: "this guy", "this person", "him", "her", etc.
  const chatWithMatch = text.match(
    /(?:i(?:'?ll| will| am going to| gonna| need to| want to)?\s+(?:be\s+)?(?:chat(?:ting)?|talk(?:ing)?|speak(?:ing)?|message|text(?:ing)?|hav(?:e|ing)\s+a\s+(?:chat|talk|convo|conversation))\s+(?:to|with)\s+|don'?t\s+(?:inter(?:rupt|fere)|respond|reply)\b|do\s+not\s+(?:inter(?:rupt|fere)|respond|reply)\b|block|ignore|silence)\s*(?:(?:this\s+)?(?:guy|person|dude|man|woman|girl|boy|friend|contact|user|buddy|bro)\s+)?\+?([\d]{7,15})/i
  );
  if (chatWithMatch) {
    const digits = chatWithMatch[chatWithMatch.length - 1].replace(/[^0-9]/g, '');
    if (digits.length >= 7) return { action: 'block', jid: `${digits}@s.whatsapp.net` };
  }

  // Fallback: any message that has a phone number AND a "do not interrupt / hands off" phrase
  // e.g. "I'll be speaking to this guy 254713046497, do not interrupt"
  // The number and the instruction can appear in either order
  const anyNumberInText = text.match(/\b(\d{7,15})\b/);
  if (anyNumberInText) {
    const lowerNoNum = lower.replace(/\d+/g, '').trim();
    const hasConversationVerb = /\b(?:speak(?:ing)?|talk(?:ing)?|chat(?:ting)?)\b.{0,25}\b(?:to|with)\b|\b(?:to|with)\b.{0,15}\b(?:speak|talk|chat)\b/i.test(text);
    const hasNoInterruptPhrase = /\b(?:don'?t|do\s+not)\s+(?:inter(?:rupt|fere)|respond|reply|butt\s+in|jump\s+in|cut\s+in)\b|no\s+interrupt|leave\s+(?:us|me)\s+alone|stay\s+out|hands\s+off/i.test(lowerNoNum);
    if (hasConversationVerb && hasNoInterruptPhrase) {
