...
// ── Runtime cache ──────────────────────────────────────────────────────────
// These are set once from the config file and then updated synchronously, so
// we never need to re-read the file on every message.
let wolfEnabled = null;  // null = not loaded yet
let _wolfConfig = null;  // full parsed config object

// ══════════════════════════════════════════════════════════════════════════
// SECTION 1 — Config file helpers
// ══════════════════════════════════════════════════════════════════════════

// Create the data directories if they don't exist yet (called before every read/write)
function ensureDirs() {
...
// Read wolf_config.json from disk.  Returns { enabled: true } as a safe default.
...
// Also refreshes the in-memory _wolfConfig cache so subsequent reads are instant.
...
// Return a summary of Wolf AI's current state (for ?wolf status).
...
// ══════════════════════════════════════════════════════════════════════════
// SECTION 4 — Per-user conversation store
// ══════════════════════════════════════════════════════════════════════════

// Load the conversation file for a user.
// Returns an empty conversation if the file doesn't exist or if it's older
// than 2 hours (conversations expire to avoid stale context).
function loadConversation(userId) {
  ...
  try {
    if (fs.existsSync(file)) {
      ...
      // Expire conversations that have been idle for more than 2 hours
...
// Save the conversation for a user.
// Trims the messages array to the last 30 turns to keep files small.
