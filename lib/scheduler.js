...
async function performScheduledUpdate(ownerJid, name) {
    ...
    // 1. Clean temp / media files (same as .restart)
    try {
        const cleanCmds = [
    ...
    // 3. Send the restart message (with code status baked in)
    try {
        await _sock.sendMessage(ownerJid, { text: buildUpdateRestart(name, gotNewCode) });
        console.log(`[scheduler] ✅ 48h update restart message sent (newCode=${gotNewCode})`);
    } catch (e) {
        console.log(`[scheduler] ❌ Failed to send update restart message: ${e.message}`);
    }

    // 4. Save all settings before exit (same as .restart)
    if (typeof globalThis.preExitSave === 'function') {
        try { await globalThis.preExitSave(); } catch {}
    }

    // 5. Restart — try pm2 first, fall back to process.exit (same as .restart)
    setTimeout(() => {
