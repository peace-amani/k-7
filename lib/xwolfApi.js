...
const MIN_MEDIA_BYTES = 50 * 1024; // 50KB minimum — anything smaller is an error page or redirect
...
export async function downloadMediaBuffer(url, timeout = 90000) {
    ...
    const proxyUrl = `https://apis.xwolf.space/proxy?url=${encodeURIComponent(url)}`;

    // Try proxy first (avoids IP block on CDN)
    try {
        ...
        const buf = res.data;
        console.log(`[downloadMediaBuffer] proxy → ${buf?.byteLength ?? 0} bytes | content-type: ${ct}`);
        if (isValidMediaBuffer(buf, ct)) return Buffer.from(buf);
        console.log(`[downloadMediaBuffer] proxy returned invalid/empty media, trying direct`);
    } catch (e) {
        console.log(`[downloadMediaBuffer] proxy error: ${e.message} — trying direct`);
    }
    ...
    try {
        ...
        const buf2 = res2.data;
        console.log(`[downloadMediaBuffer] direct → ${buf2?.byteLength ?? 0} bytes | content-type: ${ct2}`);
        if (isValidMediaBuffer(buf2, ct2)) return Buffer.from(buf2);
        console.log(`[downloadMediaBuffer] direct also returned invalid/empty media`);
    } catch (e2) {
        console.log(`[downloadMediaBuffer] direct error: ${e2.message}`);
    }
