...
export async function initSudo(botId) {
    try {
        ...
        console.log(`✅ Sudo: Loaded ${sudoData.sudoers.length} sudoers for bot ${getBotId()}, ${Object.keys(lidMap).length} LID mappings from DB`);
    } catch (err) {
...
export async function migrateSudoToSupabase() {
    ...
    try {
        ...
        console.log(`✅ Supabase: Migrated ${sudoData.sudoers.length} sudoers for bot ${getBotId()}, ${Object.keys(lidMap).length} LID mappings`);
        return true;
    } catch (err) {
