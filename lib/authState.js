...
export async function useSQLiteAuthState(db, sessionDir = './session') {
    ...
    function readKey(type, id) {
        try {
            const row = db.prepare('SELECT value FROM session_keys WHERE type = ? AND id = ? LIMIT 1').get(type, id);
            if (!row) return null;
    ...
    function readCreds() {
        try {
            const row = db.prepare("SELECT value FROM session_creds WHERE key = 'creds' LIMIT 1").get();
            if (!row) return null;
            return JSON.parse(row.value, BufferJSON.reviver);
        } catch { return null; }
    }
    ...
    async function migrateFromFiles() {
        try {
            ...
            const existing = db.prepare("SELECT 1 FROM session_creds WHERE key = 'creds' LIMIT 1").get();
            if (existing) return;
...
export function getSessionStats(db) {
    try {
        const keys = db.prepare('SELECT COUNT(*) AS c FROM session_keys').get()?.c || 0;
        const hasCreds = !!db.prepare("SELECT 1 FROM session_creds WHERE key = 'creds' LIMIT 1").get();
        const byType = db.prepare('SELECT type, COUNT(*) AS c FROM session_keys GROUP BY type').all();
