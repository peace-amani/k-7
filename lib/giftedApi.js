...
export async function mcowDownloadAudio(ytUrl) {
    // TODO: update path and params once mcow endpoint is confirmed
    const MCOW_PATH   = '/mp3';          // e.g. '/mp3', '/audio', '/download'
    const MCOW_PARAMS = { url: ytUrl };  // e.g. { url, apikey: 'gifted' }

    try {
        sigLog('🌐', 'mcow/audio', 'Attempting download…');
        const res = await axios.get(`${MCOW_BASE}${MCOW_PATH}`, {
            params:  MCOW_PARAMS,
            timeout: 20000
        });
        const d = res.data;
        ...
        sigLog('⚠️', 'mcow/audio', 'Failed (endpoint may not be active yet)', { Error: e.message }, 'yellow');
        return null;
    }
}
