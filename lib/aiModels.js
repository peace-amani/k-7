...
export const MODEL_PRIORITY = [
  ...
  'nvidia-mistral', 'deepseek', 'groq', 'llama', 'mixtral',
...
// Standard text query (xwolf AI or NVIDIA text models)
export function buildTextUrl(modelKey, query) {
  const model = AI_MODELS[modelKey];
  if (!model) return null;
  return `${XWOLF_API_BASE}/api/${model.endpoint}?q=${encodeURIComponent(query)}&key=${XWOLF_API_KEY}`;
}

// xwolf Gemini vision (base64 image)
...
// NVIDIA vision (base64 image)
export function buildNvidiaVisionUrl(endpoint, query, imageBase64) {
  const params = new URLSearchParams({ q: query, key: XWOLF_API_KEY, image: imageBase64 });
  return `${XWOLF_API_BASE}/api/${endpoint}?${params.toString()}`;
}

// NVIDIA image generation (uses `prompt` not `q`)
export function buildNvidiaImageUrl(modelKey, prompt) {
  const model = NVIDIA_IMAGE_MODELS[modelKey] || NVIDIA_IMAGE_MODELS.flux;
  const params = new URLSearchParams({ prompt, key: XWOLF_API_KEY });
  return `${XWOLF_API_BASE}/api/${model.endpoint}?${params.toString()}`;
}
