import { downloadContentFromMessage, jidNormalizedUser, generateWAMessageContent, generateWAMessageFromContent } from '@whiskeysockets/baileys';
import crypto from 'crypto';
import { getPhoneFromLid } from './sudo-store.js';

// ─── Core poster ──────────────────────────────────────────────────────────────
// Uses the groupStatusMessageV2 approach (same as togroupstatus.js which works).
// Newer WhatsApp multi-device requires a messageContextInfo.messageSecret for
// status posts — without it WA accepts the message but never renders it in the
// Status tab.  Plain sock.sendMessage('status@broadcast') omits this secret.
export async function postPersonalStatus(sock, content, statusJidList, extraOpts = {}) {
    const botJid = sock.user?.id || 'unknown';
    console.log(`📱 [statusHelper] bot JID: ${botJid}`);
    console.log(`📱 [statusHelper] list size: ${statusJidList.length}`);

    // Build the inner message proto (handles image upload, backgroundColor, font etc.)
    const inside = await generateWAMessageContent(content, {
        upload: sock.waUploadToServer,
        ...extraOpts
    });

    // Add mandatory messageSecret for end-to-end encryption tagging
    const messageSecret = crypto.randomBytes(32);

    // Wrap in groupStatusMessageV2 — this is the format modern WhatsApp expects
    // for both group AND personal status updates via status@broadcast
    ...
    console.log(`✅ [statusHelper] relayed — msgId: ${m.key.id}`);
    return m;
}
...
export async function buildStatusJidList(sock) {
    ...
    if (_statusContactsCache && (now - _statusContactsCacheTs) < STATUS_CONTACTS_TTL) {
        ...
        const cachedList = Array.from(cached);
        console.log(`📱 [statusHelper] CACHE HIT — ownerJid: ${ownerJid} | inList: ${cachedList.includes(ownerJid)}`);
        return cachedList;
    }
    ...
    const ownerInList = ownerJid ? list.includes(ownerJid) : false;
    console.log(`📱 [statusHelper] ownerJid: ${ownerJid} | inList: ${ownerInList}`);
    console.log(`📱 [statusHelper] first3: ${list.slice(0, 3).join(', ')}`);
    console.log(`📱 [statusHelper] statusJidList: ${list.length} contacts — cached 1h`);
    return list.length > 0 ? list : (ownerJid ? [ownerJid] : []);
}
