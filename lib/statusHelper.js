...
//
// Privacy tokens IQ: newer WhatsApp / Baileys RC requires marking contacts as
// "trusted" before they can see the status. Without this, WA accepts the message
// (returns a msgId) but never displays it on any recipient's Status tab.
export async function postPersonalStatus(sock, content, statusJidList, extraOpts = {}) {
    // Send privacy tokens to let WhatsApp know which contacts should see this status.
    if (typeof sock.getPrivacyTokens === 'function' && statusJidList.length > 0) {
        try {
            await sock.getPrivacyTokens(statusJidList);
        } catch (e) {
            console.log(`📱 [statusHelper] getPrivacyTokens skipped: ${e.message}`);
        }
    }
