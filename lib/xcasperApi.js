...
async function ytVideoDownload(ytUrl) {
    try {
        ...
        const dlRes = await axios.get(pick.url, {
            responseType: 'arraybuffer',
            timeout: 150_000,
            maxContentLength: 200 * 1024 * 1024,
            headers: { 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://apis.xcasper.space/' }
        });

        const buf = Buffer.from(dlRes.data);
        if (!buf || buf.length < 10_000) { console.log(`[xcasper/yt-video] buffer too small: ${buf.length}`); return null; }
