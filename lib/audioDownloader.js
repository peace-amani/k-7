...
// ── Source 4: mcow.gifted.co.ke (placeholder — ready when endpoint confirmed) ─
async function fromMcow(ytUrl) {
    const buf = await mcowDownloadAudio(ytUrl);
    if (!buf) throw new Error('mcow returned null');
    return buf;
}

// ── Source 5: XCasper yt-audio (320kbps) ─────────────────────────────────────
...
// ── Main export: fallback chain ───────────────────────────────────────────────
export async function downloadAudioWithFallback(ytUrl) {
    const sources = [
        { name: 'mcow (gifted)',   fn: () => fromMcow(ytUrl)      },  // PRIMARY — mcow.gifted.co.ke/api/yta
