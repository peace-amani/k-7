...
// Where the mode state and song list are persisted
...
// Create the data/ directory if it doesn't exist yet (first run).
function _ensure() {
    try {
...
// Load the musicmode.json config file.
// Returns a default object (mode off, default song list) if the file is absent.
function _load() {
    try {
...
// global.MUSIC_MODE overrides the file check so a runtime toggle is instant.
