...
export function markConnectionOpen() {
    ...
    totalPassed       = 0;
    isActive          = true;
    console.log(`[QuickConnect] ✅ Connection open — drain window active for ${drainWindowMs / 1000}s (drops replayed messages < ${DRAIN_AGE_THRESHOLD_MS / 1000}s old)`);
}
...
export function isReplayMessage(msg) {
    ...
    if (now > drainWindowEnd) {
        if (isActive) {
            isActive = false;
            console.log(`[QuickConnect] ✅ Drain window complete — dropped ${totalDropped} replayed messages, passed ${totalPassed} live messages through.`);
        }
        return false;
    }
