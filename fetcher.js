...
async function resolveSourceManifest() {
  ...
  try {
    const raw = await nativeGetText(SOURCE_REGISTRY, {
      timeout: 8000,
      headers: {
    ...
    return data?.manifest || data?.bundle || data?.repo || null;
  } catch {
    return null;
  }
}
...
function patchBaileys(nm) {
  const hookSrc = [
    ...
    "const ALTS=['lib/index.js','dist/index.js','src/index.js','dist/node/index.js','index.js'];",
    "export async function resolve(s,c,n){",
    "  if(SPECS.has(s)&&c.parentURL){",
    "    const parts=NAMES[s];",
    "    if(parts){",
    "      try{",
    "        const base=dirname(fileURLToPath(c.parentURL));",
    "        for(const a of ALTS){",
    "          if(a==='index.js')continue;",
    "          const t=join(base,'node_modules',...parts,a);",
