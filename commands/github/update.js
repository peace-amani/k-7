...
// const OWNER_REPO_URL = "https://github.com/WOLFTECH-Ke/silentwolf.git";
...
// Resolve the repo base URL.
// Priority: 1) lib/repoConfig.js (single source of truth)
//           2) package.json "repository.url"
//           3) base64 fallback to template repo
function _resolveOwnerRepoBase() {
  try {
    if (typeof _CFG_REPO_URL === 'string' &&
        _CFG_REPO_URL.startsWith('https://') &&
        _CFG_REPO_URL.includes('github.com')) {
      return _CFG_REPO_URL.replace(/\.git$/, '');
    }
  } catch {}
  try {
...
async function preserveEssentialFiles() {
  ...
  // Preserve just the SQLite database file (not the whole data/ dir to avoid
  // 50 MB+ ENOSPC failures on low-disk servers). If this fails we skip silently;
  // the JSON config files above are the real recovery path.
  try {
    const _dbSrc = path.join(process.cwd(), 'data', 'bot.sqlite');
    if (fs.existsSync(_dbSrc)) {
      const _dbBackupDir = path.join(preserveDir, 'data');
      await fsPromises.mkdir(_dbBackupDir, { recursive: true });
      await fsPromises.copyFile(_dbSrc, path.join(_dbBackupDir, 'bot.sqlite'));
      preserved.push('data/bot.sqlite');
    }
  } catch { /* non-critical */ }
...
async function extractZip(zipPath, outDir) {
  ...
  // ── Pure-JS fallback via adm-zip (works on Pterodactyl/Heroku) ──────────
  console.log('[extractZip] No system tool found — using adm-zip extraction');
...
export default {
  ...
  async execute(sock, m, args) {
    ...
    try {
      ...
      // ── Snapshot all live settings into .env BEFORE anything is touched ──────
      // This ensures the post-restore safety check always has fresh values to
      // recover from, even if the backup fails (ENOSPC) or the container
      // filesystem is wiped on restart (some Pterodactyl eggs reinstall on boot).
      try {
        const _envPath = path.join(process.cwd(), '.env');
        let _envContent = '';
        try { _envContent = fs.readFileSync(_envPath, 'utf8'); } catch {}
        const _envSet = (key, val) => {
          if (!val && val !== 0) return;
          const line = `${key}=${String(val)}`;
          const rx = new RegExp(`^${key}=.*$`, 'm');
          _envContent = rx.test(_envContent)
            ? _envContent.replace(rx, line)
            : _envContent.trimEnd() + '\n' + line + '\n';
          process.env[key] = String(val);
        };
        // Bot identity
        const _snapshotName = getBotName();
        if (_snapshotName && _snapshotName !== 'WOLFBOT') _envSet('BOT_NAME', _snapshotName);
        // Prefix — read from cache or prefix_config.json
        const _pfxPath = path.join(process.cwd(), 'prefix_config.json');
        try {
          const _pfxData = JSON.parse(fs.readFileSync(_pfxPath, 'utf8'));
          const _pfxVal = _pfxData.isPrefixless ? '' : (_pfxData.prefix || '');
          _envSet('BOT_PREFIX', _pfxVal !== '' ? _pfxVal : (process.env.BOT_PREFIX || ''));
        } catch { _envSet('BOT_PREFIX', process.env.BOT_PREFIX || ''); }
        // Bot mode
        const _modePath = path.join(process.cwd(), 'bot_mode.json');
        try {
          const _modeData = JSON.parse(fs.readFileSync(_modePath, 'utf8'));
          if (_modeData.mode) _envSet('BOT_MODE', _modeData.mode);
        } catch { if (process.env.BOT_MODE) _envSet('BOT_MODE', process.env.BOT_MODE); }
        // Timezone
        if (process.env.BOT_TIMEZONE) _envSet('BOT_TIMEZONE', process.env.BOT_TIMEZONE);
        fs.writeFileSync(_envPath, _envContent, 'utf8');
      } catch { /* non-critical — proceed even if snapshot fails */ }
      // ──────────────────────────────────────────────────────────────────────────
